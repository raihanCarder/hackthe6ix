...
This document consolidates `IDEA.md`, `ALGORITHM_DESIGN.md`, `GAMEPLAY_PACK_IDEAS.md`, and `ideas_dana.md` into one buildable architecture. Where those docs conflicted, the conflicts have been resolved as follows:

| Decision | Resolution |
| --- | --- |
| Primary game format | **16-team group + knockout tournament**, individual hotel matchups (per `GAMEPLAY_PACK_IDEAS.md`). The 3-card squad quick-match from `IDEA.md` is dropped from the MVP. |
| Winner determination | **Hybrid**: the recommendation engine (per `ALGORITHM_DESIGN.md`) always determines the tournament champion / final recommendation; early-round match drama uses seeded RNG for spectacle. |
| Database | **Supabase Postgres** via Prisma. |
| Stretch features | **None in MVP** — `ideas_dana.md` items (ElevenLabs, Backboard.io, Unifold) stay out of the architecture. |
...
src/
  ...
  lib/
    ...
    engine/                         # pure, framework-free — implements ALGORITHM_DESIGN.md
      ...
      types.ts                      # interfaces from ALGORITHM_DESIGN.md §15
...
**Load-bearing rule** (from `ALGORITHM_DESIGN.md`): `lib/engine/` is pure and deterministic — no DB, no fetch, no rarity, no cosmetics. `lib/game/` may read engine output but can never change metrics, weights, rankings, or the champion.

## Data model (Prisma)

Models follow `IDEA.md` with the Match model generalized to tournaments:
...
4. **Questionnaire** — 3–5 adaptive questions from the approved bank (`ALGORITHM_DESIGN.md` §3), selected deterministically based on metric coverage/variation in this pool (e.g., no price question when value is inactive). Answers → points → normalized weights. Skipped → named fallback profile ("Best overall").
...
7. **Hybrid match simulation** (`matchSim.ts`) —
   ...
   - 5–7 highlight events per match generated from real attributes only (templates from `IDEA.md`: Cancellation Shield, Transfer Window, First-Touch Finish, LEGACY attack…). Rarity/cosmetics affect visuals only.
...
2. **Stay22 vertical slice** — client, normalize, dedupe, hard filters against the real API; verify the actual response schema (rating scale, price field presence, radius units — flagged as unverified in `ALGORITHM_DESIGN.md`). This gates metric design, do it early.
3. **Engine** — metrics → weights → utilities → Monte Carlo → ranking/regret/explanations, with unit tests from `ALGORITHM_DESIGN.md` §18 (shrinkage, Haversine, determinism with fixed seed, pairwise symmetry).
