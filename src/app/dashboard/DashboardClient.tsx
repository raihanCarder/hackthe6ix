...
export function DashboardClient() {
  ...
  useEffect(() => {
    ...
    if (authMode === "auth0" && !profile) {
      window.location.assign(`/auth/login?returnTo=${encodeURIComponent("/dashboard")}`);
    }
  ...
  if (!loaded) {
    return <div className="mx-auto max-w-6xl px-4 py-14 text-center text-chalk-dim sm:px-6">Loading dashboard…</div>;
  }

  if (!profile) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center sm:px-6">
        <p className="font-display text-xl text-chalk">Sign in to see your dashboard.</p>
        <p className="mt-2 text-sm text-chalk-dim">Use the sign in button in the sidebar to get started.</p>
      </div>
    );
  }

  const bestCard =
    cards && cards.length > 0 ? [...cards].sort((a, b) => b.overall - a.overall)[0] : null;
  const bookableValue = cards?.reduce((sum, c) => sum + (c.hotel.nightlyPrice ?? 0), 0) ?? null;
  const scoreBoard = cards ? [...cards].sort((a, b) => b.overall - a.overall).slice(0, 8) : [];
  const maxScore = scoreBoard.length > 0 ? Math.max(...scoreBoard.map((c) => c.overall)) : 1;
  ...
  const tiles = [
    ...
    { label: "Bookable value", value: bookableValue !== null ? `$${bookableValue.toLocaleString()}` : "—" },
  ];
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
          ...
          <form onSubmit={mintQuickTrip} className="mt-3 grid gap-3 sm:grid-cols-2">
            <label className="sm:col-span-2">
              ...
              <input
                ...
                onChange={(e) => setQuickTrip({ ...quickTrip, destination: e.target.value })}
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) => setQuickTrip({ ...quickTrip, checkin: e.target.value })}
                className="mt-1 input-field"
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(e) => setQuickTrip({ ...quickTrip, checkout: e.target.value })}
                className="mt-1 input-field"
              />
            </label>
        ...
        <div className="panel rounded-xl p-5">
          ...
          {bestCard ? (
                ...
                <div className="mt-2 space-y-1.5">
                  {STAT_META.map(({ key, label, color }) => (
                    <div key={key} className="flex items-center gap-2 text-xs">
                      ...
                      <span className="font-score w-6 text-right text-chalk">{bestCard.stats[key]}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <p className="mt-3 text-sm text-chalk-dim">No cards yet — kick off a trip to mint your first pack.</p>
          )}
        </div>
      </div>
          ...
          <div className="mt-3 space-y-2">
            {scoreBoard.map((card) => (
              <div key={card.id} className="flex items-center gap-3 text-xs">
                ...
                <span className="font-score w-6 text-right text-chalk">{card.overall}</span>
              </div>
            ))}
          </div>
        </div>
